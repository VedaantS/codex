...
from codex.metrics.reports import InstitutionalUserReport
from codex.metrics.reporters import InstitutionalUsersReporter
...
class _InstiUserSetup:
    ...
    institution: codexdb.Institution
    now: datetime.datetime
    department_name: str | None = None
    orcid_id: str | None = None
    user: codexdb.CODEXUser = dataclasses.field(init=False)
    ...
    def _add_public_project(self) -> codexdb.Node:
        return ProjectFactory(
            creator=self.user,
            is_public=True,
        )

    def _add_private_project(self) -> codexdb.Node:
        return ProjectFactory(
            creator=self.user,
            is_public=False,
        )

    def _add_public_registration(self) -> codexdb.Registration:
        return RegistrationFactory(
            creator=self.user,
            is_public=True,
        )

    def _add_embargoed_registration(self) -> codexdb.Registration:
        return RegistrationFactory(
            creator=self.user,
            is_public=False,
    ...
    def _add_published_preprint(self) -> codexdb.Preprint | None:
