...
from codex.metrics.counted_usage import CountedAuthUsage
from codex.metrics.preprint_metrics import (
    PreprintDownload,
    PreprintView,
)
from codex.metrics.reporters.public_item_usage import PublicItemUsageReporter
from codex.metrics.reports import PublicItemUsageReport
from codex.metrics.utils import YearMonth
from codex import models as codexdb
...
class TestPublicItemUsageReporter:
    ...
    def _patch_settings(self):
        with mock.patch('website.settings.DOMAIN', 'http://codex.example'):
            yield
    ...
    def test_reporter(self, ym_empty, ym_sparse, ym_busy, sparse_month_usage, busy_month_item0, busy_month_item1, busy_month_item2, item0):
        ...
        _sparse_item0, _sparse_item1, _sparse_item2 = sorted(_sparse, key=attrgetter('item_codexid'))
        # sparse-month item0
        assert isinstance(_sparse_item0, PublicItemUsageReport)
        assert _sparse_item0.item_codexid == 'item0_v1'
        assert _sparse_item0.provider_id == [item0.provider._id]
        assert _sparse_item0.platform_iri == ['http://codex.example']
        ...
        assert _sparse_item1.item_codexid == 'item1'
        assert _sparse_item1.provider_id == ['codex']
        assert _sparse_item1.platform_iri == ['http://codex.example']
        ...
        # sparse-month item2
        assert isinstance(_sparse_item1, PublicItemUsageReport)
        assert _sparse_item2.item_codexid == 'item2'
        assert _sparse_item2.provider_id == ['codex']
        assert _sparse_item2.platform_iri == ['http://codex.example']
        ...
        # busy month:
        assert len(_busy) == 3
        _busy_item0, _busy_item1, _busy_item2 = sorted(_busy, key=attrgetter('item_codexid'))
        # busy-month item0
        assert isinstance(_busy_item0, PublicItemUsageReport)
        assert _busy_item0.item_codexid == 'item0_v1'
        assert _busy_item0.provider_id == [item0.provider._id]
        assert _busy_item0.platform_iri == ['http://codex.example']
        ...
        assert _busy_item1.item_codexid == 'item1'
        assert _busy_item1.provider_id == ['codex']
        assert _busy_item1.platform_iri == ['http://codex.example']
        ...
        assert _busy_item2.item_codexid == 'item2'
        assert _busy_item2.provider_id == ['codex']
        assert _busy_item2.platform_iri == ['http://codex.example']
...
def _save_usage(
    ...
    **kwargs,
    ...
    _countedusage_kwargs = {
        ...
        'platform_iri': 'http://codex.example',
        **kwargs,
    }
    CountedAuthUsage(**_countedusage_kwargs).save(refresh=True)
    if isinstance(item, codexdb.Preprint):
