...
from codex.metrics.reports import MonthlyReport, ReportInvalid, PublicItemUsageReport
...
class TestLastMonthReport:
    ...
    @pytest.fixture
    def this_month_report(self, codexid, this_month):
        return _item_usage_report(this_month, codexid, view_count=77)

    @pytest.fixture
    def last_month_report(self, codexid, last_month):
        return _item_usage_report(last_month, codexid, view_count=57)
    ...
    @pytest.fixture
    def two_months_back_report(self, codexid, two_months_back):
        return _item_usage_report(two_months_back, codexid, view_count=27)

    @pytest.fixture
    def three_months_back_report(self, codexid, three_months_back):
        return _item_usage_report(three_months_back, codexid, view_count=37)

    def test_with_none(self, codexid):
        assert PublicItemUsageReport.for_last_month(codexid) is None

    def test_with_others(self, codexid, this_month_report, three_months_back_report, diff_last_month_report):
        assert PublicItemUsageReport.for_last_month(codexid) is None

    def test_with_prior_month(self, codexid, this_month_report, two_months_back_report, three_months_back_report, diff_last_month_report):
        assert PublicItemUsageReport.for_last_month(codexid) == two_months_back_report

    def test_with_last_month(self, codexid, this_month_report, last_month_report, two_months_back_report, three_months_back_report, diff_last_month_report):
        assert PublicItemUsageReport.for_last_month(codexid) == last_month_report
...
def _item_usage_report(ym: YearMonth, codexid: str, **kwargs):
    _report = PublicItemUsageReport(
        report_yearmonth=ym,
        item_codexid=codexid,
        **kwargs
    )
