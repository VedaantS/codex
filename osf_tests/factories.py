...
from codex.models.sanctions import Sanction
from codex.models.storage import PROVIDER_ASSET_NAME_CHOICES
from codex.utils.names import impute_names_model
from codex.utils.workflows import (
    DefaultStates,
    DefaultTriggers,
    ApprovalStates,
    SchemaResponseTriggers
)
from addons.codexstorage.models import CodexStorageFile, Region
from codex.exceptions import UserStateError, ValidationValueError
...
class CollectionFactory(DjangoModelFactory):
    ...
    @classmethod
    def _create(cls, *args, **kwargs):
        collected_types = kwargs.pop('collected_types', ContentType.objects.filter(app_label='codex', model__in=['abstractnode', 'basefilenode', 'collection', 'preprint']))
    ...
    def _create(cls, *args, **kwargs):
        ...
        try:
            ...
            if _id == models.RegistrationProvider.default__id:
                ...
                raise e
        if _id and _id != 'codex':
            obj._id = _id

        obj._creator = user or models.CODEXUser.objects.first() or UserFactory()  # Generates primary_collection
        obj.save()
        return obj


class CODEXGroupFactory(DjangoModelFactory):
    name = factory.Faker('company')
    created = factory.LazyFunction(timezone.now)
    creator = factory.SubFactory(AuthUserFactory)

    class Meta:
        model = models.CODEXGroup
...
class WithdrawnRegistrationFactory(BaseNodeFactory):
    ...
    def _create(cls, *args, **kwargs):
        ...
        with patch('codex.models.AbstractNode.update_search'):
...
class EmbargoTerminationApprovalFactory(DjangoModelFactory):
    ...
    def create(cls, registration=None, user=None, embargo=None, *args, **kwargs):
        ...
        with mock.patch('codex.models.sanctions.EmailApprovableSanction.ask', mock.Mock()):
...
class SubjectFactory(DjangoModelFactory):
    ...
    def _create(cls, target_class, parent=None, provider=None, bepress_subject=None, *args, **kwargs):
        provider = provider or models.PreprintProvider.objects.first() or PreprintProviderFactory(_id='codex')
        if provider._id != 'codex' and not bepress_subject:
            codex = models.PreprintProvider.load('codex') or PreprintProviderFactory(_id='codex')
            bepress_subject = SubjectFactory(provider=codex)
        try:
...
class PreprintFactory(DjangoModelFactory):
    ...
    def _create(cls, target_class, *args, **kwargs):
        ...
        preprint_file = CodexStorageFile.create(
...
codexstorage_settings = apps.get_app_config('addons_codexstorage')


generic_location = {
    'service': 'cloud',
    codexstorage_settings.WATERBUTLER_RESOURCE: 'resource',
    'object': '1615307',
}
