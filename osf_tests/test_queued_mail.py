...
from codex.features import DISABLE_ENGAGEMENT_EMAILS
from codex.models.queued_mail import (
    queue_mail, WELCOME_CODEX4M,
...
class TestQueuedMail:
    ...
    @mock.patch('codex.models.queued_mail.send_mail')
    ...
    @mock.patch('codex.models.queued_mail.send_mail')
    def test_welcome_codex4m_presend(self, mock_mail, user):
        user.date_last_login = timezone.now() - dt.timedelta(days=13)
        user.save()
        mail = self.queue_mail(
            mail=WELCOME_CODEX4M,
            user=user,
    ...
    def test_disabled_queued_emails_not_sent_if_switch_active(self, user):
        with override_switch(DISABLE_ENGAGEMENT_EMAILS, active=True):
            ...
            assert self.queue_mail(mail=WELCOME_CODEX4M, user=user) is False
    ...
    def test_disabled_triggered_emails_not_sent_if_switch_active(self):
        with override_switch(DISABLE_ENGAGEMENT_EMAILS, active=True):
            ...
            assert mails.send_mail(to_addr='', mail=mails.WELCOME_CODEX4I) is False
