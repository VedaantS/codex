...
from codex.models import Guid, RegistrationSchema, Registration
from codex.models.archive import ArchiveTarget, ArchiveJob
from codex.models.base import generate_object_id
from codex.utils.migrations import map_schema_to_schemablocks
from addons.base.models import BaseStorageAddon
from api.base.utils import waterbutler_api_url_for

from codex_tests import factories
from tests.base import CodexTestCase, fake
...
mock_codexstorage = MockAddon()
mock_codexstorage.config.short_name = 'codexstorage'
mock_dropbox = MockAddon()
mock_dropbox.config.short_name = 'dropbox'

active_addons = {'codexstorage', 'dropbox'}
...
def _mock_get_addon(name, *args, **kwargs):
    ...
    if name == 'dropbox':
        return mock_dropbox
    if name == 'codexstorage':
        return mock_codexstorage
...
def use_fake_addons(func):
    ...
    def wrapper(*args, **kwargs):
        with nested(
            mock.patch('codex.models.mixins.AddonModelMixin.add_addon', mock.Mock(side_effect=_mock_get_or_add)),
            mock.patch('codex.models.mixins.AddonModelMixin.get_addon', mock.Mock(side_effect=_mock_get_addon)),
            mock.patch('codex.models.mixins.AddonModelMixin.delete_addon', mock.Mock(side_effect=_mock_delete_addon)),
            mock.patch('codex.models.mixins.AddonModelMixin.get_or_add_addon', mock.Mock(side_effect=_mock_get_or_add))
        ):
...
def generate_schema_from_data(data):
    def from_property(id, prop):
        if isinstance(prop.get('value'), dict):
            ...
            return {
                'id': id,
                'type': 'codex-upload' if prop.get('extra') else 'string',
                'format': 'codex-upload-open' if prop.get('extra') else 'text'
            }
...
def generate_metadata(file_trees, selected_files, node_index):
    uploader_types = {
        ('q_' + selected_file['name']): {
            'extra': [{
                'sha256': sha256,
                'viewUrl': f"/project/{node_index[sha256]}/files/codexstorage{selected_file['path']}",
...
class ArchiverTestCase(CodexTestCase):
    ...
    def _test__get_file_tree(self, addon_short_name):
        ...
        for path in self.URLS:
            url = waterbutler_api_url_for(
                ...
                _internal=True,
                cookie=cookie,
                base_url=self.src.codexstorage_region.waterbutler_url
            )
            responses.add(
                responses.Response(
        ...
        root = {
            ...
            'name': '',
            'kind': 'folder',
            # Regression test for CODEX-8696 confirming that size attr does not stop folders from recursing
            'size': '100',
        }
...
class TestArchiverTasks(ArchiverTestCase):
    ...
    def test_stat_addon(self):
        with mock.patch.object(BaseStorageAddon, '_get_file_tree') as mock_file_tree:
            ...
            res = stat_addon('codexstorage', self.archive_job._id)
        assert res.target_name == 'codexstorage'
    ...
    def test_archive_node_pass(self, mock_archive_addon):
        ...
        with mock.patch.object(BaseStorageAddon, '_get_file_tree') as mock_file_tree:
            mock_file_tree.return_value = FILE_TREE
            results = [stat_addon(addon, self.archive_job._id) for addon in ['codexstorage']]
        archive_node(results, self.archive_job._id)
        mock_archive_addon.assert_called_with(
            addon_short_name='codexstorage',
            job_pk=self.archive_job._id,
        )
    ...
    def test_archive_node_fail(self):
        ...
        results = [stat_addon(addon, self.archive_job._id) for addon in ['codexstorage', 'dropbox']]
    ...
    def test_archive_node_does_not_archive_empty_addons(self, mock_archive_addon, mock_send):
        with mock.patch('codex.models.mixins.AddonModelMixin.get_addon') as mock_get_addon:
    ...
    def test_archive_addon(self, mock_make_copy_request):
        archive_addon('codexstorage', self.archive_job._id)
        assert self.archive_job.get_target('codexstorage').status == ARCHIVER_INITIATED
        cookie = self.user.get_or_create_cookie()

        mock_make_copy_request.assert_called_with(
            job_pk=self.archive_job._id,
            url=f'{settings.WATERBUTLER_URL}/v1/resources/{self.src._id}/providers/codexstorage/?cookie={cookie.decode()}',
            data={
                'action': 'copy',
                'path': '/',
                'rename': 'Archive of CODEX Storage',
                'resource': self.archive_job.info()[1]._id,
                'provider': 'codexstorage',
            }
        )
        ...
        data = {
            qid: {
                'extra': [{
                    ...
                    'viewUrl': f"/project/{node._id}/files/codexstorage{fake_file['path']}",
        ...
        data = {
            ('q_' + selected['name']): {
                'extra': [{
                    ...
                    'viewUrl': f"/project/{child._id}/files/codexstorage{selected['path']}",
...
class TestArchiverUtils(ArchiverTestCase):
    ...
    def test_handle_archive_fail_copy(self, mock_send_mail):
        ...
        args_desk = dict(
            to_addr=settings.CODEX_SUPPORT_EMAIL,
            user=self.user,
            src=self.src,
...
class TestArchiverListeners(ArchiverTestCase):
    ...
    def test_archive_callback_done_success(self, mock_send, mock_archive_success):
        self.dst.archive_job.update_target('codexstorage', ARCHIVER_SUCCESS)
        self.dst.archive_job.save()
        listeners.archive_callback(self.dst)
    ...
    def test_archive_callback_done_errors(self):
        self.dst.archive_job.update_target('codexstorage', ARCHIVER_FAILURE)
        self.dst.archive_job.save()
        with mock.patch('website.archiver.utils.handle_archive_fail') as mock_fail:
            listeners.archive_callback(self.dst)
    ...
    def test_archive_callback_updates_archiving_state_when_done(self):
        ...
        reg = factories.RegistrationFactory(project=proj)
        reg.archive_job.update_target('codexstorage', ARCHIVER_INITIATED)
        child = reg.nodes[0]
        child.archive_job.update_target('codexstorage', ARCHIVER_SUCCESS)
        child.save()
    ...
    def test_archive_tree_finished_d3(self):
        ...
        for node in [reg, rchild, rchild2]:
            node.archive_job._set_target('codexstorage')
        for node in [reg, rchild, rchild2]:
            node.archive_job.update_target('codexstorage', ARCHIVER_SUCCESS)
        for node in [reg, rchild, rchild2]:
    ...
    def test_archive_tree_finished_false(self):
        ...
        for node in [reg, rchild]:
            node.archive_job.update_target('codexstorage', ARCHIVER_SUCCESS)
        rchild2.archive_job.update_target('codexstorage', ARCHIVER_INITIATED)
        rchild2.save()
        for node in [reg, rchild, rchild2]:
    ...
    def test_archive_tree_finished_false_for_partial_archive(self):
        ...
        for node in [reg, rchild]:
            node.archive_job.update_target('codexstorage', ARCHIVER_SUCCESS)
        rsibling.archive_job.update_target('codexstorage', ARCHIVER_INITIATED)
        rsibling.save()
    ...
    def test_archive_callback_on_tree_sends_only_one_email(self, mock_send_success, mock_arhive_success):
        ...
        for node in [reg, rchild, rchild2]:
            node.archive_job._set_target('codexstorage')
        for node in [reg, rchild, rchild2]:
            node.archive_job.update_target('codexstorage', ARCHIVER_INITIATED)
        rchild.archive_job.update_target('codexstorage', ARCHIVER_SUCCESS)
        rchild.save()
        listeners.archive_callback(rchild)
        assert not mock_send_success.called
        reg.archive_job.update_target('codexstorage', ARCHIVER_SUCCESS)
        reg.save()
        listeners.archive_callback(reg)
        assert not mock_send_success.called
        rchild2.archive_job.update_target('codexstorage', ARCHIVER_SUCCESS)
        rchild2.save()
...
class TestArchiverScripts(ArchiverTestCase):
    ...
    def test_find_failed_registrations(self):
        ...
        for i in range(5):
            ...
            archive_job.save()
            reg.save()
            archive_job._set_target('codexstorage')
            archive_job.update_target('codexstorage', ARCHIVER_INITIATED)
...
class TestArchiverBehavior(CodexTestCase):

    @mock.patch('codex.models.AbstractNode.update_search')
    ...
    def test_archiving_nodes_added_to_search_on_archive_success_if_public(self, mock_update_search, mock_send, mock_archive_success):
        ...
        reg.save()
        with nested(
            mock.patch('codex.models.ArchiveJob.archive_tree_finished', mock.Mock(return_value=True)),
            mock.patch('codex.models.ArchiveJob.success', mock.PropertyMock(return_value=True))
    ...
    def test_archiving_nodes_not_added_to_search_on_archive_failure(self, mock_send, mock_delete_index_node):
        ...
        reg.save()
        with nested(
                mock.patch('codex.models.archive.ArchiveJob.archive_tree_finished', mock.Mock(return_value=True)),
                mock.patch('codex.models.archive.ArchiveJob.success', mock.PropertyMock(return_value=False))
        ) as (mock_finished, mock_success):
            listeners.archive_callback(reg)
    ...
    def test_archiving_nodes_not_added_to_search_on_archive_incomplete(self, mock_send, mock_update_search):
        ...
        reg = factories.RegistrationFactory(project=proj)
        reg.save()
        with mock.patch('codex.models.ArchiveJob.archive_tree_finished', mock.Mock(return_value=False)):
            listeners.archive_callback(reg)
        assert not mock_update_search.called


class TestArchiveTarget(CodexTestCase):
...
class TestArchiveJobModel(CodexTestCase):
    ...
    def test_get_target(self):
        ...
        job.set_targets()
        codexstorage = job.get_target('codexstorage')
        assert codexstorage
    ...
    def test_set_targets(self):
        ...
        assert list(job.target_addons.values_list('name', flat=True)) == ['codexstorage']
...
# Regression test for https://openscience.atlassian.net/browse/CODEX-9085
