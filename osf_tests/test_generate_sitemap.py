...
from codex_tests.factories import (AuthUserFactory, ProjectFactory, RegistrationFactory, CollectionFactory,
...
class TestGenerateSitemap:
    ...
    @pytest.fixture(autouse=True)
    def project_preprint_codex(self, user_admin_project_public):
        return ProjectFactory(creator=user_admin_project_public, is_public=True)
    ...
    @pytest.fixture(autouse=True)
    def provider_codex(self):
        # Note: at least a provider whose _id == 'codex' have to exist for the script to work
        return PreprintProviderFactory(_id='codex', name='codexprovider')
    ...
    @pytest.fixture(autouse=True)
    def preprint_codex(self, project_preprint_codex, user_admin_project_public, provider_codex):
        return PreprintFactory(project=project_preprint_codex,
                                             creator=user_admin_project_public,
                                             provider=provider_codex)
    @pytest.fixture(autouse=True)
    def preprint_codex_blank(self, project_preprint_codex, user_admin_project_public, provider_codex):
        return PreprintFactory(project=project_preprint_codex,
                                             creator=user_admin_project_public,
                                             provider=provider_codex)

    @pytest.fixture(autouse=True)
    def preprint_codex_version(self, preprint_codex_blank):
        return PreprintFactory.create_version(create_from=preprint_codex_blank, creator=preprint_codex_blank.creator)

    @pytest.fixture(autouse=True)
    def preprint_withdrawn(self, project_preprint_codex, user_admin_project_public, provider_codex):
        preprint = PreprintFactory(project=project_preprint_codex,
                                             creator=user_admin_project_public,
                                             provider=provider_codex)
    ...
    def all_included_links(self, user_admin_project_public, user_admin_project_private, project_registration_public,
                             project_preprint_codex, project_preprint_other,
                             registration_active, provider_other, provider_codex,
                             preprint_codex, preprint_codex_version, preprint_other, preprint_withdrawn):
        ...
        urls_to_include.extend([
            ...
            project_preprint_codex.url,
            project_preprint_other.url,
            registration_active.url,
            f'/preprints/{provider_codex._id}/{preprint_codex._id}',
            f'/preprints/{provider_codex._id}/{preprint_codex_version._id}',
            f'/preprints/{provider_other._id}/{preprint_other._id}',
            f'/preprints/{provider_codex._id}/{preprint_withdrawn._id}',
            f'/{preprint_codex._id}/download/?format=pdf',
            f'/{preprint_codex_version._id}/download/?format=pdf',
