...
from codex.management.commands.clear_expired_sessions import clear_expired_sessions
from codex_tests.factories import AuthUserFactory
from codex.exceptions import InvalidCookieOrSessionError
from codex.models import UserSessionMap
from tests.base import fake, AppTestCase
from website import settings as codex_settings
...
class TestSessions(AppTestCase):
    ...
    def setUp(self):
        ...
        self.fake_url = 'http://fake.codex.io/fake'
        ...
        self.cookie = itsdangerous.Signer(codex_settings.SECRET_KEY).sign(session.session_key)
        ...
        self.cookie_session_anonymous = itsdangerous.Signer(codex_settings.SECRET_KEY).sign(session_anonymous.session_key)
        # Invalid Session
        session_invalid = SessionStore()
        session_invalid.create()
        self.cookie_session_invalid = itsdangerous.Signer(codex_settings.SECRET_KEY).sign(session_invalid.session_key)
        # Others
        self.cookie_session_removed = itsdangerous.Signer(codex_settings.SECRET_KEY).sign(fake.md5())
    ...
    def test_drf_get_session_from_cookie_with_session_removed(self):
        ...
        assert ret_val.session_key == itsdangerous.Signer(codex_settings.SECRET_KEY).unsign(self.cookie_session_removed).decode()
        assert ret_val.load() == {}
    ...
    def test_flask_get_session_from_cookie_with_session_expired(self):
        ...
        cookie_session_expired = itsdangerous.Signer(codex_settings.SECRET_KEY).sign(session_expired.session_key)
        with pytest.raises(InvalidCookieOrSessionError):
...
class TestSessionUtils:
    ...
    @mock.patch('codex.models.UserSessionMap.objects.filter')
