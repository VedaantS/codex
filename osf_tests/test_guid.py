...
from codex.models import Guid, GuidVersionsThrough, NodeLicenseRecord, CODEXUser, Preprint
from codex.models.base import VersionedGuidMixin
from codex.utils.permissions import ADMIN
...
class TestGuid:
    ...
    def test_loading_by_short_guid(self):
        obj = UserFactory()
        assert CODEXUser.load(obj._id) == obj
...
class TestResolveGuid(CodexTestCase):
    ...
    @mock.patch('codex.models.node.Node.deep_url', None)
    ...
    def test_resolve_guid_no_auth_redirect_to_cas_includes_public_with_url_segments(self):
        ...
        public
        """
        for segment in ('comments', 'links', 'components', 'files', 'files/codexstorage', 'files/addon'):
    ...
    def test_resolve_guid_private_request_access_or_redirect_to_cas(self):
        ...
        assert '<title>CODEX | Forbidden</title>' in res.text
        ...
        assert res.status_code == 403
        assert '<title>CODEX | Request Access</title>' in res.text
    ...
    def test_resolve_guid_download_file(self):
        ...
        pp.primary_file.create_version(
            creator=pp.creator,
            location={'folder': 'codex', 'object': 'deadbe', 'service': 'cloud'},
