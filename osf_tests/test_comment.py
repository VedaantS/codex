...
from codex.models import Comment, NodeLog, Guid, BaseFileNode
...
class TestCommentModel:
    ...
    def test_create_sends_mention_added_signal_if_group_member_mentions(self, node, user, auth):
        ...
        group = CODEXGroupFactory(creator=manager)
        node.add_codex_group(group)
...
class FileCommentMoveRenameTestMixin:
    id_based_providers = ['codexstorage']
    ...
    def test_comments_move_when_file_moved_to_codexstorage(self, project, user):
        codexstorage = project.get_addon('codexstorage')
        root_node = codexstorage.get_root()
        codex_file = root_node.append_file('file.txt')
        codex_file.create_version(user, {
            'object': '06d80e',
            'service': 'cloud',
        ...
        file_node = BaseFileNode.resolve_class('codexstorage', BaseFileNode.FILE).get_or_create(destination['node'], destination['path'])
        assert self.guid._id == file_node.get_guid()._id
        file_comments = Comment.objects.filter(root_target=self.guid.pk)
        assert file_comments.count() == 1

    def test_comments_move_when_folder_moved_to_codexstorage(self, project, user):
        codexstorage = project.get_addon('codexstorage')
        root_node = codexstorage.get_root()
        codex_folder = root_node.append_folder('subfolder')
        codex_file = codex_folder.append_file('file.txt')
        codex_file.create_version(user, {
            'object': '06d80e',
            'service': 'cloud',
            codexstorage_settings.WATERBUTLER_RESOURCE: 'codex',
        ...
        file_name = 'file.txt'
        self._create_file_with_comment(node=source['node'], path='{}{}'.format(source['path'], file_name), user=user)
        payload = self._create_payload('move', user, source, destination, self.file._id, destination_file_id=codex_file._id)
        update_file_guid_referent(self=None, target=destination['node'], event_type='addon_file_moved', payload=payload)
        self.guid.reload()

        file_node = BaseFileNode.resolve_class('codexstorage', BaseFileNode.FILE).get_or_create(destination['node'], codex_file._id)
        assert self.guid._id == file_node.get_guid()._id
        file_comments = Comment.objects.filter(root_target=self.guid.pk)
        assert file_comments.count() == 1
...
# copied from tests/test_comments.py
class TestCodexstorageFileCommentMoveRename(FileCommentMoveRenameTestMixin):

    provider = 'codexstorage'
    ProviderFile = CodexStorageFile
    ...
    def _create_file_with_comment(self, node, path, user):
        codexstorage = node.get_addon(self.provider)
        root_node = codexstorage.get_root()
    ...
    def test_comments_move_when_file_moved_to_codexstorage(self):
        # Already in CODEXStorage
        pass

    def test_comments_move_when_folder_moved_to_codexstorage(self):
        # Already in CODEXStorage
        pass
...
PROVIDER_CLASS = {
    'codexstorage': TestCodexstorageFileCommentMoveRename,
