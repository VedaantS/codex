...
from codex.models import PageCounter, CODEXGroup

from tests.base import CodexTestCase
from codex_tests.factories import UserFactory, ProjectFactory

SessionStore = import_module(django_conf_settings.SESSION_ENGINE).SessionStore

class TestAnalytics(CodexTestCase):
...
def file_node(project):
    file_node = CodexStorageFile(name='test', target=project)
    file_node.save()
    return file_node

@pytest.fixture()
def file_node2(project):
    file_node2 = CodexStorageFile(name='test2', target=project)
    file_node2.save()
    return file_node2

@pytest.fixture()
def file_node3(project):
    file_node3 = CodexStorageFile(name='test3', target=project)
    file_node3.save()
    return file_node3
...
class TestPageCounter:
    ...
    def test_download_update_counter_contributor(self, user, project, file_node):
        ...
        platform_group = CODEXGroup.objects.create(creator=user, name='Platform')
        group_member = UserFactory()
        project.add_codex_group(platform_group)
